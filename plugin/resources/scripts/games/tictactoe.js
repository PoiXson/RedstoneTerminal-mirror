...
function loop() {
	...
	for (let ix=0; ix<screen_width;  ix++) pixels.set(y,   ix,  Color.WHITE);
	for (let ix=0; ix<screen_width;  ix++) pixels.set(y*2, ix,  Color.WHITE);
	for (let iy=0; iy<screen_height; iy++) pixels.set(iy,  x,   Color.WHITE);
	for (let iy=0; iy<screen_height; iy++) pixels.set(iy,  x*2, Color.WHITE);
